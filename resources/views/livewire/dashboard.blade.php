<div>

    @foreach ($users as $key => $value)
        <div>
            {{ $value->name }}
        </div>
    @endforeach

    <div class="mx-10">

        <x-filament::pagination :paginator="$users" :page-options="[10, 20, 50, 100, 'all']" :current-page-option-property="'perPage'" />
    </div>


    <div class="flex items-center p-4">

        <x-filament::avatar
            src="data:image/jpeg;base64,"
            alt="Dan Harrin" :circular="false" :size="'w-40 h-40'" :class="'mx-10'" />

        <x-filament::loading-indicator class="h-7 w-7 mx-10 text-success" />
        <x-filament::modal>
            <x-slot name="trigger">
                <x-filament::button :class="'mx-10 bg-black hover:bg-white font-semibold hover:text-black py-2 px-4 border border-solid rounded'" icon="heroicon-m-sparkles">
                    Open modal
                </x-filament::button>
            </x-slot>

            <h1>text</h1>
            {{-- Modal content --}}
        </x-filament::modal>
        <x-filament::link :href="route('components.filament.pagination')" :class="'bg-black p-2 rounded'" wire:navigate color="success">
            link
        </x-filament::link>
        <x-filament::badge color="success" :class="'mx-10'" icon="heroicon-m-sparkles" icon-position="after">
            Badge
        </x-filament::badge>

        <x-filament::breadcrumbs :breadcrumbs="[
            '/' => 'Home',
            '/dashboard' => 'Dashboard',
            '/dashboard/users' => 'Users',
            '/dashboard/users/create' => 'Create User',
        ]" />

        <x-filament::button badge-color="danger" :class="'mx-10'" color="gray" icon="heroicon-m-sparkles"
            icon-position="after">
            Mark notifications as read

            <x-slot name="badge">
                3
            </x-slot>
        </x-filament::button>
    </div>
    <div class="flex items-center p-4">

        <x-filament::input.checkbox wire:model="isAdmin" :valid="!$errors->has('isAdmin')" color="success" />

        <x-filament::dropdown :class="'mx-10'">
            <x-slot name="trigger">
                <x-filament::button color="danger" :class="' hover:bg-white font-semibold hover:text-black py-2 px-4 border border-solid rounded'">
                    More actions
                </x-filament::button>
            </x-slot>

            <x-filament::dropdown.list :class="'w-40'">
                <x-filament::dropdown.list.item wire:click="openViewModal">
                    View
                </x-filament::dropdown.list.item>

                <x-filament::dropdown.list.item wire:click="openEditModal">
                    Edit
                </x-filament::dropdown.list.item>

                <x-filament::dropdown.list.item wire:click="openDeleteModal">
                    Delete
                </x-filament::dropdown.list.item>
            </x-filament::dropdown.list>
        </x-filament::dropdown>

        <x-filament::fieldset>
            <x-slot name="label">
                Address
            </x-slot>

            <input type="datetime" name="date" id="date" class="w-full p-2 border border-gray-300 rounded" />

            <x-filament::button icon="heroicon-m-sparkles" :class="'mt-3'" color="info">
                Reset
            </x-filament::button>
            <x-filament::button icon="heroicon-m-sparkles" :class="'mx-2 mt-3'" color="success">
                Submit
            </x-filament::button>
        </x-filament::fieldset>

        <x-filament::icon-button icon="heroicon-m-plus" wire:click="openNewUserModal" label="New label"
            :class="'px-1 w-96'" :size="'xl'" />
    </div>
</div>
